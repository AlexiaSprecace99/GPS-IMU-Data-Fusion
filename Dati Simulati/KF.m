...
plot(Tc,vera_acc(1,:),'c', LineWidth=1); hold on;  grid on;
plot(Tc,log_EKF.x_hat(7,:),'b', LineWidth=1); hold on;  grid on;
legend('Real North acceleration','Measured North acceleration','estimated North acceleration');%,'measured North acceleration');
xlabel('T[s]'); ylabel('Acceleration[m/s^{2}]')

figure;
plot(Tc,acceleration_gen(2,:),'r', LineWidth=1.5);hold on;
plot(Tc,vera_acc(2,:),'c', LineWidth=1); hold on;  grid on;
plot(Tc,log_EKF.x_hat(8,:),'b', LineWidth=1); hold on; grid on;
legend('Real East acceleration','Measured East acceleration','estimated East acceleration');%,'measured East acceleration');
xlabel('T[s]'); ylabel('Acceleration[m/s^{2}]')

figure;
plot(Tc,acceleration_gen(3,:),'r', LineWidth=1.5); hold on;
plot(Tc,vera_acc(3,:),'c', LineWidth=1); hold on;  grid on;
plot(Tc,log_EKF.x_hat(9,:),'b',LineWidth=1); hold on; grid on;
%plot(Tc,vera_acc(3,:),'b', LineWidth=1); hold on;  grid on;
legend('Real Down acceleration','Measured Down acceleration','estimated Down acceleration');%,'measured Down acceleration');
xlabel('T[s]'); ylabel('Acceleration[m/s^{2}]')
